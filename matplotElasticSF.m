...
function [fig,h,max_x,max_y] = matplotElasticSF(fig,h,ictr,x1,y1,x2,y2,max_x,max_y,nspec,ifile,varargin)
...
%     nirow = ceil(sqrt(npspec)); njcol = ceil(sqrt(npspec)); %Creates SQUARE Matrix Plot
    nirow = 4; njcol = 4; %Allows Overwriting of Matrix Plot Configuration
%Defines Plot Handle:
...
%Plots Data:
    %Plots Vertical Force at Column Face (Experimental):
        ...
        xlswrite('MatplotDataElastic-Rev02.xlsx',{sprintf('subplot(%d,%d,%d):\n',nirow,njcol,ictr)},'SF_Meas',sprintf('%s1',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',{'x1'},'SF_Meas',sprintf('%s2',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',{'y1'},'SF_Meas',sprintf('%s2',alphabet(2*(ictr-1)+2)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',x1,'SF_Meas',sprintf('%s3',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',y1,'SF_Meas',sprintf('%s3',alphabet(2*(ictr-1)+2)));
        
        xlswrite('MatplotDataElastic-Rev02.xlsx',{sprintf('subplot(%d,%d,%d):\n',nirow,njcol,ictr)},'SF_Fitted',sprintf('%s1',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',{'x2'},'SF_Fitted',sprintf('%s2',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',{'y2'},'SF_Fitted',sprintf('%s2',alphabet(2*(ictr-1)+2)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',x2,'SF_Fitted',sprintf('%s3',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',y2,'SF_Fitted',sprintf('%s3',alphabet(2*(ictr-1)+2)));
...
if ictr == nspec
    ...
    %Creates Matrix Plot Configuration:
        %Defines Uniform Axis Limits:
            axis(h,[-0.025*max_x,0.020,-0.035*max_y,6.50]);
    ...
    %Annotates Label Onto Plot:
        for secspec = 1:nspec
            ...
            %Annotates Plot:
                text(0,5.96,sprintf('%s',vlist{secspec}),'fontname','arial','fontsize',10,...
                    'fontweight','bold','horizontalalignment','left','verticalalignment','middle','interpreter','none');
                set(gca,'xtick',0:0.005:0.020);
%                 xticks = get(gca,'xtick');
%                 set(gca,'xticklabel',[sprintf('0\n'),sprintf('%0.3f\n',xticks(2:end))]);
                set(gca,'ytick',0:1:8);
        end
    %Applies Super-Labels:
        [~,hx] = suplabel('Engineering Strain (mm/mm)','x'); %For Journal Article);
        set(hx,'fontname','arial','fontsize',12); %For BA Journal Article
        [~,hy] = suplabel('Engineering Stress (MPa)','y');
        set(hy,'fontname','arial','fontsize',12);
