function f = user_FiveParameterObjectiveFunction(x,e,s)

%Unpacks Parameters
    ki = x(1); ky = x(2); ry = x(3); ny = x(4); eo = x(5);
...
%Richard Equation:
    R = (ki-ky)*(e-eo)./(1+abs(((ki-ky).*(e-eo))./ry).^ny).^(1/ny) + ky*(e-eo);
%     R(d>do) = (kb-kp)*(d(d>do)-dbr)./(1+abs(((kb-kp).*(d(d>do)-dbr))/rb).^nb).^(1/nb) + kp*(d(d>do)-dbr);
%Functional:
    f = trapz(e,abs(s-R)); %transpose needed because P and R are both column vectors
