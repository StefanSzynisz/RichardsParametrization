...
function [fig,h,max_x,max_y] = matplotSF(fig,h,ictr,x1,y1,x2,y2,max_x,max_y,nspec,ifile,varargin)
...
%Plots Data:
    %Plots Vertical Force at Column Face (Experimental):
        ...
        xlswrite('MatplotData-Rev02.xlsx',{'x1'},'SF_Meas',sprintf('%s2',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotData-Rev02.xlsx',{'y1'},'SF_Meas',sprintf('%s2',alphabet(2*(ictr-1)+2)));
        xlswrite('MatplotData-Rev02.xlsx',x1,'SF_Meas',sprintf('%s3',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotData-Rev02.xlsx',y1,'SF_Meas',sprintf('%s3',alphabet(2*(ictr-1)+2)));
        
        xlswrite('MatplotData-Rev02.xlsx',{sprintf('subplot(%d,%d,%d):\n',nirow,njcol,ictr)},'SF_Fitted',sprintf('%s1',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotData-Rev02.xlsx',{'x2'},'SF_Fitted',sprintf('%s2',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotData-Rev02.xlsx',{'y2'},'SF_Fitted',sprintf('%s2',alphabet(2*(ictr-1)+2)));
        xlswrite('MatplotData-Rev02.xlsx',x2,'SF_Fitted',sprintf('%s3',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotData-Rev02.xlsx',y2,'SF_Fitted',sprintf('%s3',alphabet(2*(ictr-1)+2)));
%         pause
