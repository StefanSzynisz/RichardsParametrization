...
function [fig,h,max_x,max_y] = matplotElasticAF(fig,h,ictr,x1,y1,x2,y2,max_x,max_y,nspec,ifile,varargin)
...
%Plots Data:
    %Plots Vertical Force at Column Face (Experimental):
        ...
        xlswrite('MatplotDataElastic-Rev02.xlsx',{'x1'},'AF_Meas',sprintf('%s2',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',{'y1'},'AF_Meas',sprintf('%s2',alphabet(2*(ictr-1)+2)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',x1,'AF_Meas',sprintf('%s3',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',y1,'AF_Meas',sprintf('%s3',alphabet(2*(ictr-1)+2)));
        
        xlswrite('MatplotDataElastic-Rev02.xlsx',{sprintf('subplot(%d,%d,%d):\n',nirow,njcol,ictr)},'AF_Fitted',sprintf('%s1',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',{'x2'},'AF_Fitted',sprintf('%s2',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',{'y2'},'AF_Fitted',sprintf('%s2',alphabet(2*(ictr-1)+2)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',x2,'AF_Fitted',sprintf('%s3',alphabet(2*(ictr-1)+1)));
        xlswrite('MatplotDataElastic-Rev02.xlsx',y2,'AF_Fitted',sprintf('%s3',alphabet(2*(ictr-1)+2)));
        
%         fprintf('subplot(%d,%d,%d):\n',nirow,njcol,ictr);
%         fprintf(repmat('%1.4f', 1, length(x1)), x1);
%         pause
...
if ictr == nspec
    ...
    %Creates Matrix Plot Configuration:
        %Defines Uniform Axis Limits:
            axis(h,[-0.025*max_x,0.020,-0.035*max_y,24.0]);
    ...
    %Annotates Label Onto Plot:
        for secspec = 1:nspec
            ...
            %Annotates Plot:
                text(0,22.0,sprintf('%s',vlist{secspec}),'fontname','arial','fontsize',10,...
